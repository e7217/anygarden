...
from doorae.db.models import Machine, MachineActivityLog, MachineEngine, MachineToken
...
async def _authenticate_machine(
    ...
    """Validate machine token from ``Sec-WebSocket-Protocol: doorae.v1, bearer.<token>``."""
    token: str | None = None
...
async def ws_machine(websocket: WebSocket, machine_id: str) -> None:
    ...
    selected_subprotocol: str | None = "doorae.v1" if raw_protocols else None
