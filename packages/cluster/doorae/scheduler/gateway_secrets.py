...
SDKs route through the doorae reverse proxy at
...
def build_engine_secrets(
    *,
    ...
    auth it had before — the same behaviour as pre-#359 doorae):
    ...
    - ``cluster_external_url`` is missing/empty — without a reachable
      doorae URL there's nothing to put in ``BASE_URL``, so degrade
    ...
    pointing at the doorae reverse proxy. The agent token doubles as
    ...
    return {
        ...
        # mount prefix; doorae reverse-proxies that under
...
def openhands_model_id_for_gateway(provider: str, model_name: str) -> str | None:
    ...
    ``provider/model`` prefixes. doorae's gateway exposes
