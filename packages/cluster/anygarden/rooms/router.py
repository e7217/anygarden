...
async def add_participant(
    ...
    # Both branches funnel through ``anygarden.rooms.membership`` so the
...
async def remove_participant(
    ...
    if manager is not None:
        from anygarden.ws.protocol import RoomMembershipChangedOut
...
async def get_room_token_stats(
    ...
    Response shape (see ``anygarden.rooms.token_stats`` for details)::
...
async def delete_room_artifact(
    ...
    if connection_manager is not None:
        from anygarden.ws.protocol import RoomArtifactRemovedOut
