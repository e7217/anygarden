...
# of anygarden wrote into ``.claude/settings.json`` which is only read
...
# ── anygarden self-MCP default entry (#277) ────────────────────────


# Reserved name for the built-in anygarden self-MCP entry that the
...
ANYGARDEN_BUILTIN_NAME = "anygarden"
...
ANYGARDEN_TOKEN_ENV_VAR = "ANYGARDEN_AGENT_TOKEN"


def anygarden_default_entry(
    *,
    engine: str,
    cluster_url: str,
    agent_token: str,
) -> RenderedInstance | None:
    """Return the anygarden self-MCP entry rendered for *engine*.
