...
anygarden-server runs when ``ANYGARDEN_LLM_GATEWAY_ENABLED=true``. Its
responsibilities:
...
1. Spawn the subprocess with ``--config <rendered-yaml>`` and a clean
   ...
   ``ANYGARDEN_LITELLM_*`` names to avoid colliding with anygarden's own
...
Production wiring lives in ``anygarden.app`` where the factories are
...
# ``AnygardenSettings.llm_gateway_health_timeout_sec`` /
# ``ANYGARDEN_LLM_GATEWAY_HEALTH_TIMEOUT_SEC``.
...
class _SpawnParams:
    ...
    # (``ANYGARDEN_LITELLM_*`` only) so a stray inherited var can't shadow.
...
class LLMGatewaySupervisor:
    ...
    Instantiated once by :mod:`anygarden.app` during ``lifespan`` when
