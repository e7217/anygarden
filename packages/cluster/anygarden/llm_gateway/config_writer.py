...
credential is emitted as ``os.environ/ANYGARDEN_LITELLM_<env_var_name>``
...
# Prefixing with ``ANYGARDEN_LITELLM_`` isolates our keys from the
...
_ENV_PREFIX = "ANYGARDEN_LITELLM_"
...
def _rewrite_ollama_provider(upstream: str) -> str:
    ...
    Operationally: there is no scenario in anygarden where ``ollama/``
    ...
    *,
    ...
    Args:
        ...
        master_key_env: env var name the subprocess should read its
            ...
            ``ANYGARDEN_LITELLM_MASTER_KEY=<ephemeral>`` in the child's
    ...
    body = {
        ...
        "general_settings": {
            ...
            # Postgres via Prisma. anygarden logs usage itself in its
