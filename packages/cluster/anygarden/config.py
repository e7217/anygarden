...
class AnygardenSettings(BaseSettings):
    ...
    mcp_secrets_key: str = ""
    # #197 — Embedded LiteLLM gateway. When enabled, anygarden-server
    ...
    # ``~/.anygarden/litellm.yaml`` — resolved lazily in the gateway
    ...
    # DB so the default SQLite ``anygarden.db`` stays compact as rooms
    ...
    def reachable_host(self) -> str:
        ...
        ``ANYGARDEN_HOST`` to the server's real hostname explicitly.
        """
