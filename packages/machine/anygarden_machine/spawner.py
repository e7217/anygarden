...
class SpawnManifest:
    ...
    # build time. Codex's ``[mcp_servers.anygarden] bearer_token_env_var``
...
class Spawner:
    ...
    def _migrate_legacy_workspace(cls, agent_root: Path, engine: str) -> None:
        ...
        if marker.is_file() and engine == "codex-cli":
            return
    ...
    def _materialize_agent_dir(self, msg: SpawnManifest) -> Path:
        ...
        # session. Keep the machine-level anygarden-agent cwd collapsed to
    ...
    async def spawn(self, msg: SpawnManifest) -> SpawnResult:
        ...
        - Passes agent_token via ANYGARDEN_TOKEN env var only (never argv)
        ...
        try:
            fd, tmp_path = tempfile.mkstemp(
                prefix=f"anygarden-agent-{agent_id}-", suffix=".yaml"
            )
        ...
        # Build environment: inherit current env + set ANYGARDEN_TOKEN.
        #
        # ``engine_secrets`` are deliberately NOT merged into this env.
        # ``anygarden-agent`` would inherit them and an LLM tool call
        ...
        # by ``anygarden_agent.secrets.load_from_stdin()`` at startup —
        # the agent process's /proc/self/environ stays clean (#184).
        #
        # ``ANYGARDEN_TOKEN`` stays in env because the agent's auth
        # identity token is a single anygarden-internal credential with
        ...
        # variable so codex can resolve the anygarden self-MCP entry at
        ...
        if msg.anygarden_mcp_token:
            env["ANYGARDEN_AGENT_TOKEN"] = msg.anygarden_mcp_token
        ...
        # reads the JSON payload once at startup (``anygarden_agent.secrets
