"""Local manifest persistence layer for anygarden-machine.
...
``anygarden_mcp_token`` (#277 — the anygarden self-MCP bearer token, also
...
from anygarden_machine.safefs import safe_write_text

logger = logging.getLogger(__name__)

# Fields stripped before writing to disk.
_EXCLUDED_FIELDS: frozenset[str] = frozenset(
    {"type", "engine_secrets", "anygarden_mcp_token"}
)
...
class ManifestStore:
    ...
    agents_root:
        ...
        (``<agents_root>/<agent_id>/manifest.json``).
        Defaults to ``~/.anygarden/agents``.
    """

    def __init__(self, agents_root: Path | None = None) -> None:
        self.agents_root: Path = (
            agents_root if agents_root is not None else Path.home() / ".anygarden" / "agents"
        )
        ...
        # Issue #277 — same in-memory pattern for the anygarden self-MCP
        ...
        # state transitions.
        self._anygarden_token_cache: dict[str, str] = {}
    ...
    def save(self, frame: SyncDesiredStateFrame) -> Path:
        ...
        # #277 — cache the anygarden self-MCP bearer token in memory so
        # the reconcile loop can hand it to the spawner without
        # round-tripping through disk.
        if frame.anygarden_mcp_token:
            self._anygarden_token_cache[frame.agent_id] = frame.anygarden_mcp_token
        else:
            self._anygarden_token_cache.pop(frame.agent_id, None)
    ...
    def delete(self, agent_id: str) -> None:
        ...
        self._anygarden_token_cache.pop(agent_id, None)
    ...
    def get_anygarden_mcp_token(self, agent_id: str) -> str | None:
        """Return the cached anygarden self-MCP bearer token (#277).

        Same lifetime semantics as ``get_secrets``: ``None`` until the
        cluster pushes a fresh ``sync_desired_state`` frame after a
        daemon restart. Spawner injects this as ``ANYGARDEN_AGENT_TOKEN``
        in the agent process env so codex's ``bearer_token_env_var``
        indirection resolves correctly.
        """
        return self._anygarden_token_cache.get(agent_id)
