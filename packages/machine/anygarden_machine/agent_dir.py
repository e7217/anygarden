...
- **No agent-id escape**: ``msg.agent_id`` is used as a path segment
  under ``~/.anygarden/agents/``, so a malicious server that sends
...
# lockstep with ``anygarden/agent_files.py``. Only add entries with
...
_ALLOWED_EXTENSIONS: frozenset[str] = frozenset(
    {
        ...
        # server-side whitelist in ``anygarden/agent_files.py``.
        ".sh", ".py", ".js", ".ts", ".mjs",
    }
)
...
# concatenate with a directory. Anygarden itself generates UUID4 strings
...
def validate_agent_file_path(path: str) -> None:
    ...
    server-side copy in ``anygarden_server/agent_files.py``.
    """
    if not path:
