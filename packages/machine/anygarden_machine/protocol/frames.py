...
class SyncDesiredStateFrame(BaseModel):
    ...
    # ``ANYGARDEN_AGENT_PERMISSION_LEVEL`` in the agent process env;
    ...
    # value as ``ANYGARDEN_AGENT_TOKEN`` in the agent process env.
...
class RotateTokenFrame(BaseModel):
    ...
    persists the new token to ``~/.anygarden/machine.token`` and uses it
