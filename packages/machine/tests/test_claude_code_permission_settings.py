...
``permissions.allow``. Restricted agents lose Bash/Write/Edit/Task
so the LLM can still inspect (Read/Glob/Grep/WebSearch/WebFetch) but
not execute shell commands or mutate the workspace. Standard and
trusted both keep the pre-#309 broad allow-list because claude-code
has no OS-level sandbox — Bash already lets the agent escape the
workspace, so there is no separate ``trusted`` dial to relax.
"""
