...
class TestSpawn:
    ...
    async def test_spawn_sets_codex_home_for_codex_engine(
        self, spawner: Spawner, tmp_path: Path
    ) -> None:
        """codex 엔진은 ``.codex/config.toml`` 을 cwd 기준이 아니라
        ``$CODEX_HOME/config.toml`` 기준으로만 탐색한다. doorae 가
        materialize 한 per-agent ``.codex/config.toml`` 을 codex
        app-server가 실제로 읽게 하려면 ``CODEX_HOME`` 을 해당 경로로
        리다이렉트해야 한다. 이걸 빼먹으면 MCP 템플릿이 ``.codex/``
        에 정확히 떨어져도 codex 는 사용자 홈의 ``~/.codex/`` 로
        fallback 하고 MCP 가 실제로 활성화되지 않는다.
        """
        msg = SpawnManifest(
        ...
        # 대상 디렉토리는 materialize 단에서 이미 만들어져 있어야
        # 한다 — codex가 그 안에 auth.json, history.jsonl 등을 쓸
        # 수 있어야 하므로.
