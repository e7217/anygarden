...
_DEFAULT_TOKEN_FILE = Path.home() / ".doorae" / "agent.token"


def load_token(
    *,
    cli_token: str | None = None,
    env_var: str = "DOORAE_TOKEN",
    token_file: Path | None = None,
) -> str:
    """Resolve an authentication token using the following priority:

    1. Explicit ``cli_token`` (from ``--token`` flag)
    2. Environment variable (default ``DOORAE_TOKEN``)
    3. Token file (default ``~/.doorae/agent.token``)
