"""Private secrets storage for the anygarden-agent process.

The machine daemon pipes per-agent ``engine_secrets`` (API keys and
similar credentials) to ``anygarden-agent`` via stdin at startup. The
...
* ``anygarden-agent``'s ``/proc/self/environ`` must stay clean. A
...
def load_from_stdin() -> None:
    ...
    Called once at ``anygarden-agent`` startup by the CLI entry point.
    ...
    * Non-dict JSON is rejected to avoid pollution from arbitrary
    ...
    if not isinstance(data, dict):
        return
    for key in ("AG_ENGINE_ENDPOINT_CONFIG", "AG_ENGINE_ENDPOINT_KEY", "AG_PI_NATIVE_AUTH_CONFIG", "AG_PI_NATIVE_AUTH_KEY"):
...
def env_with_secrets(
    ...
    process sees the API keys while the parent ``anygarden-agent`` stays
    clean.
    """
