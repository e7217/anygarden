# Runbook: OpenHands agent + Ollama via doorae LLM gateway (#359)

> 대상: 외부 provider API key 없이 로컬 Ollama 만으로 OpenHands agent 를 동작시키려는 운영자.

## 사전 조건

- doorae cluster + machine daemon 정상 동작 중
- Ollama 인스턴스가 reachable. 기본은 `http://192.168.100.97:11434` (또는 운영자 환경의 ollama URL)
- `openhands-sdk` 가 머신의 doorae-agent venv 에 설치되어 있음 (#357 detector 가 advertise 가능)
- doorae 가 #359 머지 이후 빌드

## 활성화 절차

### 1. 환경변수 설정

doorae-server 가 두 변수를 읽어야 합니다. shell, systemd unit, docker env 등 어디에 두든 무관:

```bash
export DOORAE_LLM_GATEWAY_ENABLED=true
export DOORAE_CLUSTER_EXTERNAL_URL=http://localhost:8001
```

`DOORAE_CLUSTER_EXTERNAL_URL` 은 agent 프로세스가 reverse proxy 를 부를 주소입니다. 단일 머신 dev 환경이라면 `http://localhost:8001`. 머신 daemon 이 별도 머신에서 돈다면 그 머신에서 reachable 한 doorae-server URL 로 바꾸세요.
...
async def main():
    engine = create_async_engine("sqlite+aiosqlite:////home/$USER/.doorae/doorae.db")
...
doorae-server 의 reverse proxy 가 살아있고 litellm 까지 reach 하는지:
...
> **CLI 로 만들 때 주의**: `--model` 인자는 \`openai/qwen3.6:27b\` (provider prefix 포함). 이 prefix 는 OpenHands SDK 가 litellm 을 통해 doorae 의 OpenAI-compat reverse proxy 로 라우팅하는 신호입니다 — 실제 backend 는 ollama 지만 agent 입장에서는 OpenAI 와 똑같은 모양으로 보입니다.
...
- 점검:
  1. `DOORAE_LLM_GATEWAY_ENABLED=true` 가 doorae-server 환경에 진짜 들어갔는지 (`echo $DOORAE_LLM_GATEWAY_ENABLED` 또는 systemd `EnvironmentFile`)
  2. `DOORAE_CLUSTER_EXTERNAL_URL` 이 빈 문자열이 아닌지 (`""` 도 가드에 걸려서 빈 dict 반환됨)
...
- 점검: doorae-server 부팅 로그에서 `llm_gateway.spawn_failed` 검색. 흔한 케이스:
