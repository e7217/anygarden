# Week 1: anygarden-server 뼈대

> **목표**: `uvx anygarden-server`를 실행하면 5초 이내에 WebSocket 채팅이 동작하는 서버
> **산출물**: `anygarden-server/` 저장소, 단위 테스트 40개, SQLite DB 자동 생성
...
Week 1은 **anygarden-server 저장소를 처음부터 생성**하고, 다음 컴포넌트를 갖춘 뼈대를 만든다:
- FastAPI + uvicorn 앱
...
- [ ] `anygarden-server/` 디렉토리 생성
- [ ] `pyproject.toml` 작성 (08-operations.md §8.2.1 정본 복사)
- [ ] `alembic.ini` 작성 (async SQLite 설정)
- [ ] `anygarden/__init__.py`, `anygarden/__main__.py` 생성
- [ ] `anygarden/config.py` 작성 — Pydantic Settings:
  ```python
  class AnygardenSettings(BaseSettings):
      host: str = "127.0.0.1"
      port: int = 8000
      db_url: str = f"sqlite+aiosqlite:///{Path.home()}/.anygarden/anygarden.db"
...
- [ ] `anygarden/db/engine.py` — `create_async_engine` + `async_sessionmaker`
- [ ] `anygarden/db/models.py` — 7개 엔티티:
...
- [ ] `anygarden/db/repository.py` — `append_message()` with seq 발급, `replay_since_seq()`
- [ ] `anygarden/db/migrations/env.py` — async Alembic 설정
- [ ] `anygarden/db/migrations/versions/001_initial.py` — 자동 생성 (`alembic revision --autogenerate`)
- [ ] **검증**: `pytest tests/test_models.py` — 엔티티 CRUD + 제약조건 (10개 테스트)

### Phase 1C: 인증 계층 (Day 2 오전)

- [ ] `anygarden/auth/jwt.py`:
...
- [ ] `anygarden/ws/protocol.py` — Pydantic v2 프레임 모델:
  ```python
  ...
  class OutgoingFrame(BaseModel):
      ...
      # ... 타입별 필드
  ```
- [ ] `anygarden/ws/manager.py` — ConnectionManager:
...
- [ ] `anygarden/ws/handler.py`:
  - `@router.websocket("/ws/rooms/{room_id}")`
  - `Sec-WebSocket-Protocol` subprotocol 인증 → `websocket.accept(subprotocol="anygarden.v1")`
...
- [ ] `anygarden/app.py` — FastAPI factory:
  ```python
  def create_app(config: AnygardenSettings) -> FastAPI:
      app = FastAPI(title="Anygarden", lifespan=lifespan)
      app.include_router(ws_router)
      app.add_middleware(...)
      return app
  ```
  - lifespan: DB engine 생성 + migration 자동 적용 + JWT secret 자동 생성
- [ ] `anygarden/observability/metrics.py` — 5개 Prometheus 지표
- [ ] `anygarden/observability/logging.py` — structlog 설정
- [ ] `anygarden/cli.py`:
  - `@click.group(invoke_without_command=True)` → `main()`
  - `--host`, `--port`, `--db`, `--config`, `--log-level` 옵션
  - 첫 실행 시 `~/.anygarden/` 자동 생성 + JWT secret 자동 발급
  - `anygarden-server init`, `anygarden-server migrate` 서브커맨드
- [ ] **검증**: `pytest tests/test_config.py` + 수동 `uvx anygarden-server` 실행
...
- [ ] `ruff check anygarden tests` — 린트 통과
- [ ] `mypy anygarden` — 타입 체크 통과
- [ ] 테스트 커버리지 목표 80%
...
- [ ] `uvx anygarden-server`가 5초 이내에 `[INFO] Anygarden server listening on http://127.0.0.1:8000` 출력
- [ ] 첫 실행 시 `~/.anygarden/anygarden.db` 자동 생성 + migration 적용
- [ ] `~/.anygarden/config.toml` 자동 생성 + JWT secret 자동 발급
- [ ] WebSocket `/ws/rooms/{room_id}` 접속 + 메시지 송수신 동작
- [ ] `Sec-WebSocket-Protocol: anygarden.v1, bearer.<token>` 인증 동작
- [ ] 쿼리 파라미터 `?token=...` 거부
- [ ] `since_seq` 재연결 복구 동작
- [ ] `pytest` 40개 테스트 통과
