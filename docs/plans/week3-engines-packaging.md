...
> **산출물**: 4종 엔진 동작, `uvx anygarden-{server,agent,client}` 공개 가능
...
### anygarden-sdk 엔진 어댑터 (4종 추가)

```
anygarden-sdk/anygarden_sdk/integrations/
...
```
anygarden-sdk/examples/profiles/
...
```
anygarden-server/.github/workflows/
├── test.yml                     # pytest + ruff + mypy
└── release-pypi.yml             # 태그 시 PyPI 배포

anygarden-sdk/.github/workflows/
...
- [ ] `anygarden_sdk/cli.py` 최종 정리:
  - `--engine` 6종 선택 (claude-code, codex, openhands, deep-agents, openai, anthropic)
  - `--profile` 파일 로드
  - `--model` 옵션 (엔진별)
  - `--room` 복수 지정 가능
  - 도움말 깔끔하게
- [ ] `anygarden_sdk/cli.py`의 `client_main`:
  - `anygarden-client --server ws://... --user me --room sprint-42`
  - `anygarden-client admin init --email ...` → REST API 호출로 초기 유저 생성
  - `anygarden-client admin token create --name PM --engine claude-code` → REST API
- [ ] **검증**: `anygarden-agent --help`, `anygarden-client --help` 출력 확인

### Phase 3C: CI/CD 워크플로 (Day 4)

- [ ] `anygarden-server/.github/workflows/test.yml` (§8.7.1 정본)
  - Python 3.11 + 3.12 매트릭스
  - pytest + ruff + mypy
- [ ] `anygarden-server/.github/workflows/release-pypi.yml` (§8.7.3 정본)
  - 태그 `v*` 시 `python -m build` + `pypa/gh-action-pypi-publish`
- [ ] `anygarden-sdk/.github/workflows/test.yml` (§8.7.2 정본)
  - protocol-compat 잡 포함 (서버 저장소의 protocol.py와 해시 비교)
- [ ] `anygarden-sdk/.github/workflows/release-pypi.yml`
...
- [ ] `uvx anygarden-server` + `uvx anygarden-agent` + `uvx anygarden-client` 동작 확인
...
- [ ] `uvx anygarden-server` → `uvx anygarden-agent --engine openai` → `uvx anygarden-client` 3자 대화 동작
- [ ] PyPI에 `anygarden-server` + `anygarden-sdk` 배포
...
- Standalone 모드만 가능 (사용자가 직접 `uvx anygarden-agent` 실행)
