# Week 4: doorae-machine 패키지 (Machine Daemon)

> **목표**: `uvx doorae-machine register` + `uvx doorae-machine run`이 동작하는 Daemon
> **산출물**: `doorae-machine/` 저장소, Daemon이 서버에 등록되고 spawn 명령을 수신 가능
...
Week 4는 **3번째 저장소 `doorae-machine/`을 처음부터 생성**하고, Machine Daemon을 구현한다:
- 엔진 capability 자동 감지 (6종)
- 서버와 WebSocket(`/ws/machines/{id}`) 연결 + register + heartbeat
- `spawn_agent` 명령 수신 → `uvx doorae-agent` subprocess spawn
...
│   ├── config.py                            # [30 LOC] ~/.doorae/machine.toml + .token
...
- [ ] `doorae-machine/` 디렉토리 생성
- [ ] `pyproject.toml` 작성 (§10.3 정본: websockets, httpx, pydantic, click, structlog, psutil, pyyaml, argon2-cffi)
- [ ] `doorae_machine/__init__.py` — `__version__ = "0.1.0"`
- [ ] `doorae_machine/config.py`:
  - `~/.doorae/machine.toml` 로드 (machine_id, name, server_url, limits)
  - `~/.doorae/machine.token` 분리 파일 (chmod 600)
- [ ] `doorae_machine/protocol/frames.py`:
...
- [ ] `doorae_machine/detector.py` (§10.4 코드 기반):
...
- [ ] `doorae_machine/daemon.py` (§10.5 코드 기반):
  ...
  - **WebSocket 인증**: `Sec-WebSocket-Protocol: doorae.v1, bearer.<machine_token>` (쿼리 파라미터 금지)
- [ ] **검증**: `pytest tests/test_daemon.py` — mock WS 서버와 register/heartbeat 5개 테스트

### Phase 4D: Spawner + Supervisor (Day 3)

- [ ] `doorae_machine/spawner.py` (§10.6 코드 기반):
  - `spawn(msg) -> SpawnResult`: 
    - profile_yaml을 `/tmp/doorae-agent-{id}.yaml`로 저장 (chmod 600)
    - 토큰은 **환경변수 `DOORAE_TOKEN`으로만** 전달 (argv 금지, ps aux 노출 방지)
    - `asyncio.create_subprocess_exec(["uvx", "--from", "doorae-sdk[{engine}]", "doorae-agent", ...])`
...
- [ ] `doorae_machine/cli.py`:
  - `doorae-machine register --server URL --name NAME`:
    1. 유저 인증 (email/password → JWT 획득)
    2. `detect_engines()` 실행
    3. `POST /api/v1/machines` → machine_id + machine_token 수신
    4. `~/.doorae/machine.toml` + `~/.doorae/machine.token` 저장
  - `doorae-machine run`:
    1. config 로드
  ...
  - `doorae-machine install-systemd-unit`:
    - `~/.config/systemd/user/doorae-machine.service` 파일 생성 (§10.10 정본)
    - `systemctl --user daemon-reload` 안내
- [ ] **검증**: `doorae-machine --help` 출력, mock 기반 register 테스트
...
- [ ] 수동 E2E:
  ...
  2. `uvx doorae-machine register --server http://localhost:8000 --name test-box`
  3. `uvx doorae-machine run` → "Connected, waiting for spawn commands"
  4. 서버에서 수동으로 spawn_agent 프레임 전송 → Daemon이 agent subprocess 띄움
  5. Agent가 `/ws/rooms/X`에 접속하여 채팅 참여 확인
- [ ] `ruff check doorae_machine tests` + `mypy doorae_machine`
...
- [ ] Machine Token은 `~/.doorae/machine.token` 파일로 분리 저장 (config.toml에 넣지 않음)
- [ ] `machine.token` 파일 chmod 600
- [ ] WebSocket 인증은 `Sec-WebSocket-Protocol: doorae.v1, bearer.<token>` (쿼리 파라미터 금지)
- [ ] Agent subprocess에 토큰은 환경변수 `DOORAE_TOKEN`으로만 전달 (argv 금지)
...
- [ ] `uvx doorae-machine register --server http://localhost:8000` 동작
- [ ] `uvx doorae-machine run` 실행 시 서버에 "online" 상태로 등록
- [ ] `uvx doorae-machine status` 출력
