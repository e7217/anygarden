...
현재 에이전트 라이프사이클은 **명령형(imperative) 모델**로, anygarden-server가 모든 결정을 내리고 anygarden-machine은 수동적 실행기(command executor)이다.
...
양쪽(anygarden-machine, anygarden-server) 공통 프레임 모델 정의.
anygarden-machine의 `protocol/frames.py`와 anygarden-server의 WS 핸들러에 새 프레임 추가.

### Phase 2: anygarden-machine 변경
