...
> **목표**: Room CRUD + 오케스트레이션 + anygarden-sdk ChatClient + Claude Code/OpenAI 엔진 통합
> **산출물**: anygarden-server 완성, anygarden-sdk/ 저장소 생성, 통합 테스트 20개
...
- **anygarden-sdk** 저장소 생성 + ChatClient (WebSocket 래퍼 + 재연결)
...
├── anygarden_sdk/
│   ├── __init__.py
│   ├── cli.py                       # [80 LOC] anygarden-agent + anygarden-client CLI
│   ├── client.py                    # [120 LOC] ChatClient (WebSocket + 재연결)
│   ├── protocol/
│   │   ├── __init__.py
│   │   ├── frames.py                # [50 LOC] 서버 anygarden/ws/protocol.py 복사본
...
│   └── profile/
│       ├── __init__.py
│       ├── loader.py                # [30 LOC] ~/.anygarden/agents/*.yaml
│       └── schema.py                # [20 LOC]
└── tests/
    ├── conftest.py
...
- [ ] `anygarden/messages/router.py`:
  - `GET /api/v1/rooms/{id}/messages?since_seq=N&limit=50` — 페이지네이션
- [ ] `anygarden/messages/service.py`:
...
- [ ] `anygarden/ws/handler.py` 확장: 오케스트레이션 룰 적용
- [ ] **검증**: 오케스트레이션 테스트 5개 (쿨다운, 멘션, typing)

### Phase 2C: anygarden-sdk 스캐폴딩 + ChatClient (Day 3)

- [ ] `anygarden-sdk/` 저장소 생성
- [ ] `pyproject.toml` 작성 (§8.3.1 정본)
- [ ] `anygarden_sdk/protocol/frames.py` — 서버 `anygarden/ws/protocol.py`에서 복사
- [ ] `anygarden_sdk/auth/token.py` — 환경변수 `ANYGARDEN_TOKEN` 로드
- [ ] `anygarden_sdk/client.py` — ChatClient:
...
- [ ] `anygarden_sdk/integrations/base.py` — EngineAdapter ABC:
  ```python
  ...
  ```
- [ ] `anygarden_sdk/integrations/claude_code.py`:
...
- [ ] `anygarden_sdk/integrations/openai.py`:
...
### Phase 2E: anygarden-agent CLI + 프로필 로더 (Day 5)

- [ ] `anygarden_sdk/profile/loader.py` — `~/.anygarden/agents/*.yaml` 로더
- [ ] `anygarden_sdk/profile/schema.py` — Pydantic 프로필 스키마 (name, engine, system_prompt, rooms, mcp_servers)
- [ ] `anygarden_sdk/cli.py`:
  - `anygarden-agent --engine claude-code --name PM --server ws://... --token $TOK`
  - `anygarden-client --server ws://... --user me --room sprint-42`
...
- [ ] 메시지 히스토리 페이지네이션 동작
- [ ] `uvx anygarden-agent --engine openai --name Host` 실행 시 채팅 참여 동작
- [ ] `uvx anygarden-client --user me --room test` 실행 시 메시지 송수신 동작
- [ ] 쿨다운 + 멘션 라우팅 기본 동작
