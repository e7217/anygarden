...
|---|---|---|---|---|
| **doorae-server** | `doorae-server/` | `doorae` | `doorae-server` | `doorae-server` |
| **doorae-sdk** | `doorae-sdk/` | `doorae_sdk` | `doorae-sdk` | `doorae-agent`, `doorae-client` |
| **doorae-machine** | `doorae-machine/` | `doorae_machine` | `doorae-machine` | `doorae-machine` |
...
3. Machine에는 `doorae-machine`(Daemon)과 `doorae-sdk`(Agent subprocess용)만 설치되며, 서버 의존성(SQLAlchemy, Alembic 등)이 딸려오지 않는다
4. Machine Daemon은 머신 호스트에 상주하는 별도 배포물이며 서버·SDK와 배포 사이클이 다르다
5. 역방향 의존성 없음: 세 패키지 모두 서로를 런타임 의존하지 않는다

세 저장소 중 **서버와 SDK는 `protocol/frames.py` 파일을 문자 그대로 복사하여 공유**한다. 양쪽 CI가 파일 해시 비교 테스트(`tests/test_protocol_compat.py`)를 돌려 깨지면 실패시킨다. Machine 저장소는 별도의 Machine↔Server 프레임(`doorae_machine/protocol/frames.py`)을 가지며, 이는 §10의 프로토콜을 따른다.

Python 패키지 이름(`doorae-server`, `doorae-sdk`, `doorae-machine`)은 하이픈을 쓰고, Python import 이름(`doorae`, `doorae_sdk`, `doorae_machine`)은 언더스코어 관례를 따른다 (PEP 8).

> **`doorae-client`는 `doorae-sdk` 패키지에 속한다**. 유저 클라이언트가 서버 의존성(SQLAlchemy, Alembic, prometheus-client 등)을 필요로 하지 않기 때문이다. 관리 기능(`doorae-client admin init`, `doorae-client admin token create` 등)은 서버의 **REST API를 HTTP로 호출**하는 방식이며, 서버 코드가 로컬에 있을 필요가 없다.

## 8.2 `doorae-server` 패키지

### 8.2.1 pyproject.toml

```toml
# doorae-server/pyproject.toml
[build-system]
requires = ["hatchling"]
...
[project.scripts]
doorae-server = "doorae.cli:main"

[project.urls]
Homepage = "https://github.com/doorae/doorae"
Documentation = "https://docs.doorae.io"

[tool.hatch.build.targets.wheel]
packages = ["doorae"]  # 루트의 doorae/ 디렉토리 (src-layout 아님)
```
...
Options:
  ...
  --db TEXT             DB URL [기본: sqlite+aiosqlite:///~/.doorae/doorae.db]
  --config PATH         설정 파일 [기본: ~/.doorae/config.toml]
...
```python
# doorae-server/doorae/cli.py
# 서버 전용 CLI. `doorae-client`는 doorae-sdk 패키지에 속한다 (§8.3.5).
import click
import uvicorn
...
def _initial_setup(config):
    ...
    config_dir = Path("~/.doorae").expanduser()
    config_dir.mkdir(parents=True, exist_ok=True)
    (config_dir / "agents").mkdir(exist_ok=True)
    (config_dir / "logs").mkdir(exist_ok=True)
    config.save(config_dir / "config.toml")
    click.echo(f"Initialized Doorae at {config_dir}")
...
### 8.2.3 설정 파일 (`~/.doorae/config.toml`)
...
```bash
$ uvx doorae-server
[INFO] Initializing Doorae at /home/me/.doorae
[INFO] Generated JWT secret
[INFO] Created database: /home/me/.doorae/doorae.db
[INFO] Running migrations... done (3 migrations applied)
[INFO] Doorae server v0.1.0 listening on http://127.0.0.1:8000
...
```toml
# doorae-sdk/pyproject.toml
[project]
name = "doorae-sdk"
version = "0.1.0"
description = "Doorae multi-agent chat SDK and CLI"
requires-python = ">=3.11"
...
[project.scripts]
doorae-agent = "doorae_sdk.cli:agent_main"
doorae-client = "doorae_sdk.cli:client_main"
```
...
```bash
# Claude Code 사용자
uvx --from "doorae-sdk[claude-code]" doorae-agent --engine claude-code ...

# Codex 사용자
uvx --from "doorae-sdk[codex]" doorae-agent --engine codex ...

# 여러 엔진 혼용
uvx --from "doorae-sdk[all]" doorae-agent --engine claude-code ...
```

### 8.3.2 SDK 구조

SDK는 **별도 저장소 `doorae-sdk/`**이며, import 경로는 `doorae_sdk.*`이다 (서버의 `doorae.*`와 구분).

```
doorae-sdk/                     # 별도 PyPI 패키지 저장소
├── pyproject.toml              # name = "doorae-sdk", packages = ["doorae_sdk"]
├── README.md
├── doorae_sdk/                 # Python 패키지 루트
│   ├── __init__.py
│   ├── cli.py                  # doorae-agent CLI 엔트리
│   ├── client.py               # ChatClient (WebSocket 기반)
│   ├── protocol/
│   │   ├── __init__.py
│   │   ├── frames.py           # 서버 doorae/ws/protocol.py와 문자 그대로 동일
...
│   └── profile/
│       ├── __init__.py
│       ├── loader.py           # ~/.doorae/agents/*.yaml 로드
...
```python
# doorae-sdk/doorae_sdk/client.py
...
```python
# doorae-sdk/doorae_sdk/integrations/claude_code.py
from __future__ import annotations
...
### 8.3.5 에이전트 CLI (`doorae-agent`)

```python
# doorae-sdk/doorae_sdk/cli.py
import asyncio
import click
from pathlib import Path
...
@click.option("--token", envvar="DOORAE_TOKEN", required=True)
...
def agent_main(engine, name, server, token, profile, model, room):
    ...
    예:
        doorae-agent --engine claude-code --name PM
        doorae-agent --engine codex --name TechLead --room sprint-42
        doorae-agent --engine openai --model gpt-4o --name Host
    """
...
# PyInstaller 단일 바이너리(§8.4.3) 시나리오에서 `pyinstaller doorae_sdk/cli.py`로
# 이 파일이 스크립트로 실행될 때 필요. uvx/PyPI 경로는 pyproject.toml의
# `[project.scripts]` 엔트리(`doorae-agent = "doorae_sdk.cli:agent_main"`)가
# 직접 호출하므로 불필요.
if __name__ == "__main__":
    agent_main()
```

`doorae-client` CLI도 이 SDK 패키지(`doorae_sdk.cli:client_main`)에서 제공된다 (§8.3.1 pyproject.toml 참조). 유저 클라이언트가 서버 의존성(SQLAlchemy, Alembic 등)을 가져올 필요가 없으므로, SDK의 가벼운 의존성(websockets, httpx, click)만으로 충분하다. 관리 기능(`doorae-client admin ...`)은 서버의 REST API를 HTTP(httpx)로 호출하는 방식이다.

### 8.3.6 에이전트 프로필 YAML

```yaml
# ~/.doorae/agents/pm.yaml
name: PM
role: project_manager
engine: claude-code
system_prompt: |
  당신은 Doorae 프로젝트의 PM입니다.
...
# 터미널 2: 에이전트 1
DOORAE_TOKEN=agt_xxx uvx --from "doorae-sdk[claude-code]" \
    doorae-agent --engine claude-code --name PM --room sprint-42

# 터미널 3: 에이전트 2
DOORAE_TOKEN=agt_yyy uvx --from "doorae-sdk[openai]" \
    doorae-agent --engine openai --model gpt-4o --name Host --room sprint-42

# 터미널 4: 유저 (doorae-client는 doorae-sdk 패키지가 제공 — §8.3.1)
DOORAE_USER_TOKEN=usr_zzz uvx --from doorae-sdk doorae-client \
...
```ini
# ~/.config/systemd/user/doorae-server.service
[Unit]
Description=Doorae Chat Server
After=network-online.target

[Service]
Type=simple
ExecStart=%h/.local/bin/uvx doorae-server --host 0.0.0.0 --port 8000
Restart=on-failure
RestartSec=5
MemoryMax=512M
StandardOutput=append:%h/.doorae/logs/server.log
StandardError=append:%h/.doorae/logs/server-err.log
...
```ini
# ~/.config/systemd/user/doorae-agent-pm.service
[Unit]
Description=Doorae PM Agent
After=doorae-server.service
Requires=doorae-server.service

[Service]
Type=simple
Environment="DOORAE_TOKEN=agt_xxxxx"
ExecStart=%h/.local/bin/uvx --from "doorae-sdk[claude-code]" \
    doorae-agent --engine claude-code --name PM \
    --server ws://127.0.0.1:8000 \
    --profile %h/.doorae/agents/pm.yaml
Restart=on-failure
RestartSec=10
MemoryMax=256M
...
systemctl --user enable doorae-server doorae-agent-pm
systemctl --user start doorae-server doorae-agent-pm
...
```bash
# doorae-server/scripts/build-binary.sh
#!/usr/bin/env bash
set -euo pipefail
...
# 현재 저장소(doorae-server) + 자주 쓰이는 SDK 통합을 함께 번들링
pip install -e ".[binary]"
pip install "doorae-sdk[claude-code,openai,anthropic]"
...
ls -lh dist/
# dist/doorae-server   (~25-45MB)
# doorae-client는 doorae-sdk 패키지에서 빌드된다 (아래 SDK 빌드 스크립트 참조)
```

SDK(에이전트 CLI) 저장소는 별도로 빌드한다:

```bash
# doorae-sdk/scripts/build-binary.sh
#!/usr/bin/env bash
set -euo pipefail
...
pyinstaller \
    --onefile \
    --name doorae-agent \
    --collect-all doorae_sdk \
    doorae_sdk/cli.py

ls -lh dist/
# dist/doorae-agent    (~30-50MB, 엔진 의존성 포함 시 더 큼)
```
  ...
  build:
    ...
    steps:
      ...
      - uses: softprops/action-gh-release@v1
        with:
          files: dist/doorae-server*
          fail_on_unmatched_files: true
```

**설치 스크립트** (`https://get.doorae.io`):
...
mkdir -p "$INSTALL_DIR"
for bin in doorae-server doorae-agent doorae-client; do
...
```bash
$ curl -sSL https://get.doorae.io | sh
...
|---|---|---|---|---|
| A. 로컬 dev | 개발자 | <10초 | uv 캐시 ~100MB | `uv tool upgrade doorae-server` |
| B. systemd VPS | 운영 | ~30초 | ~200MB | `systemctl --user restart doorae-server` 후 자동 최신 |
...
```python
# doorae-server/doorae/observability/metrics.py
...
MESSAGE_FANOUT_DURATION = Histogram(
    "doorae_message_fanout_duration_seconds",
...
groups:
  - name: doorae
    rules:
      - alert: DooraeHighMessageLatency
        expr: histogram_quantile(0.99, rate(doorae_message_fanout_duration_seconds_bucket[5m])) > 0.1
        for: 5m
        annotations:
          summary: "메시지 라우팅 p99 지연이 100ms 초과"

      - alert: DooraeHighErrorRate
        expr: rate(doorae_errors_total[5m]) > 0.1
        for: 5m
        annotations:
          summary: "에러율 0.1/s 초과"

      - alert: DooraeDBSlow
        expr: histogram_quantile(0.99, rate(doorae_db_write_duration_seconds_bucket[5m])) > 0.05
        for: 10m
        annotations:
          summary: "DB write p99 지연이 50ms 초과 — SQLite → PostgreSQL 승격 고려"

      - alert: DooraeWSConnectionDrop
        expr: delta(doorae_ws_active_connections[1m]) < -10
        annotations:
...
```python
# doorae-server/doorae/observability/logging.py
import structlog
import logging
...
import pytest
from doorae.messages.service import MessageService
...
from doorae.config import load_config
...
서버와 SDK는 **별도 저장소**이므로 각자 워크플로를 갖는다. 두 저장소는 프로토콜 호환성 테스트를 공유해야 하므로, SDK 저장소의 CI가 `doorae-server`의 `protocol/frames.py` 원본을 가져와 해시 비교하는 단계를 포함한다.

### 8.7.1 `doorae-server` 저장소 — 테스트

```yaml
# doorae-server/.github/workflows/test.yml
name: Test
...
jobs:
  test:
    ...
    steps:
      ...
      - run: pytest tests/ -v --cov=doorae
      - run: ruff check doorae tests
      - run: mypy doorae
```

### 8.7.2 `doorae-sdk` 저장소 — 테스트 + 프로토콜 호환성

```yaml
# doorae-sdk/.github/workflows/test.yml
name: Test
on: [push, pull_request]
...
jobs:
  test:
    ...
    steps:
      ...
      - run: pytest tests/ -v --cov=doorae_sdk
      - run: ruff check doorae_sdk tests
      - run: mypy doorae_sdk
  ...
  protocol-compat:
    ...
    steps:
      ...
      - uses: actions/checkout@v4
        with:
          repository: doorae/doorae-server
          path: server
          ref: main
      - run: |
          diff -q \
              sdk/doorae_sdk/protocol/frames.py \
              server/doorae/ws/protocol.py \
...
```yaml
# doorae-server/.github/workflows/release-pypi.yml
name: Release doorae-server to PyPI
on:
  push:
    tags: ['v*']
jobs:
...
`doorae-sdk/.github/workflows/release-pypi.yml`은 동일한 구조이며, 내부 패키지 이름만 다르다. 두 저장소는 독립적으로 버전 번호를 올린다 — 꼭 같은 버전을 유지할 필요는 없지만, `protocol-compat` 테스트가 깨지면 해당 번호로 릴리즈되지 않는다.
...
- `doorae-server`, `doorae-agent`, `doorae-client` CLI 완성
- PyPI 패키징 (`pyproject.toml`)
- Prometheus 지표 5개 (Machine 추가 2종은 Week 5에서)
- 문서 + 첫 릴리즈 v0.1.0 (채팅 단독)

### 8.8.4 Week 4: Machine Daemon 패키지 (`doorae-machine`)
- `doorae_machine/detector.py` (6종 엔진 감지: binary 3 + python 1 + env 2)
- `doorae_machine/spawner.py` (subprocess, chmod 600 프로필, env 토큰)
- `doorae_machine/supervisor.py` (watchdog)
- `doorae_machine/daemon.py` (WS 재연결 + heartbeat 30s)
...
- `doorae/scheduler/{placement,lifecycle,machine_bus}.py`
- `/ws/machines/{id}` 핸들러 + `auth/machine_token.py`
- `doorae/api/v1/machines.py` + `doorae/api/v1/agents.py` 선언적 생성 REST
- Prometheus 지표 2종 추가 (`doorae_machines_online`, `doorae_agents_by_state`)
...
- [ ] `uvx doorae-server`가 5초 이내에 "listening on ..." 출력
- [ ] 첫 실행 시 `~/.doorae/` 디렉토리 자동 생성
...
- [ ] `doorae-server --help` / `doorae-agent --help` / `doorae-machine --help` 깔끔한 출력
- [ ] systemd unit 템플릿 문서 존재
- [ ] README 빠른 시작 3분 이내 따라할 수 있음
- [ ] `uvx doorae-machine register` 가 토큰 발급 + `~/.doorae/machine.token` 저장
- [ ] `uvx doorae-machine run` 가 서버에 online 상태로 등록됨
...
§10에서 도입된 `doorae-machine` 패키지는 **별도 저장소**이며 서버와 독립적으로 릴리즈된다.

### 8.10.1 패키지 구조 요약

```
doorae-machine/                 # 3번째 독립 PyPI 패키지
├── pyproject.toml              # name="doorae-machine", import=doorae_machine
├── doorae_machine/
...
│   ├── config.py               # ~/.doorae/machine.toml + .token 분리
...
```bash
# 등록 (1회)
uvx doorae-machine register --server https://doorae.example.com --name dev-box-1

# foreground 실행
uvx doorae-machine run
...
# [각 Machine마다]
$ uvx doorae-machine register --server https://doorae.example.com --name alice-laptop
$ uvx doorae-machine install-systemd-unit
$ systemctl --user enable --now doorae-machine

# [다른 Machine]
$ uvx doorae-machine register --server https://doorae.example.com --name gpu-box-1
$ systemctl --user enable --now doorae-machine

# [admin 머신에서 선언적 생성]
$ uvx doorae-client agent create --engine claude-code --name PM --room sprint-42
