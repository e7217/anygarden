...
|---|---|---|---|---|
| **유저** | JWT (HS256) | `Authorization: Bearer ...` 헤더 (HTTP) 또는 `Sec-WebSocket-Protocol: anygarden.v1, bearer.<token>` (WS) | 24시간 | 이메일+비밀번호 재로그인 |
...
from anygarden.db.models import AgentToken  # 별도 테이블
...
### 5.2.3 `anygarden/auth/dependencies.py`
...
from anygarden.auth.jwt import verify_user_token, InvalidToken
from anygarden.auth.token import resolve_agent_token
...
async def get_identity(
    ...
    if authorization and authorization.lower().startswith("bearer "):
        ...
        # 예: "anygarden.v1, bearer.eyJhbGciOi..."
...
```python
# anygarden/rooms/router.py
from fastapi import APIRouter, Depends
from anygarden.auth.dependencies import require_room_member, require_room_admin
...
uvx anygarden-client admin token create \
...
```python
# anygarden/rooms/router.py 내 list_messages 쿼리 확장
...
| 단일 VPS | `wss://anygarden.example.com` | Let's Encrypt (Caddy 자동) |
| K8s | `wss://...` | cert-manager |

**nginx/Caddy 프록시 설정 예시** (Caddy 권장, 자동 TLS):

```caddy
anygarden.example.com {
...
그래서 이 구현은 WebSocket 인증을 **`Sec-WebSocket-Protocol` subprotocol 헤더**로만 받는다. 구체적으로는 WebSocket handshake 시 클라이언트가 subprotocol 리스트에 `anygarden.v1`과 함께 `bearer.<token>`을 포함시킨다.

**브라우저 (표준 WebSocket API)**:
```javascript
// 쿼리 파라미터 NO
// const ws = new WebSocket("wss://anygarden.example.com/ws/rooms/abc?token=..."); // 금지

// Sec-WebSocket-Protocol YES
const ws = new WebSocket(
    "wss://anygarden.example.com/ws/rooms/abc",
    ["anygarden.v1", `bearer.${token}`]
);
```
...
websocat 'wss://anygarden.example.com/ws/rooms/abc' \
    --protocol 'anygarden.v1,bearer.eyJhbGciOi...'

# 또는 -H로 직접 헤더 지정
websocat 'wss://anygarden.example.com/ws/rooms/abc' \
    -H 'Sec-WebSocket-Protocol: anygarden.v1, bearer.eyJhbGciOi...'
```

**서버 측**:
- `anygarden/auth/dependencies.py`의 `get_identity`가 `Sec-WebSocket-Protocol` 헤더에서 `bearer.` prefix를 가진 항목을 추출한다 (§5.2.3 코드 참조).
- 핸드셰이크 응답 시 선택한 subprotocol(`anygarden.v1`)을 반환해야 브라우저가 연결을 수락한다:
  ```python
...
| JWT secret | 파일 (`~/.anygarden/jwt.secret`, 600 권한) |
| DB 파일 | 파일 시스템 권한 (`~/.anygarden/anygarden.db`, 600) |
| 로그 | 토큰·비밀번호를 절대 로깅하지 않음 (structlog processor에서 redact) |

**`structlog` redaction 설정** (`anygarden/observability/logging.py`):
...
이 경계가 모호해지면 서버 보안 모델이 감당할 수 없이 복잡해진다. Anygarden 서버는 "메시지 허브"이지 "에이전트 샌드박스 오케스트레이터"가 아니다.

---

## 5.10 구현 체크리스트

- [ ] `anygarden/auth/jwt.py`: `create_user_token`, `verify_user_token` (~40줄)
- [ ] `anygarden/auth/token.py`: `generate_token`, `hash_token`, `resolve_agent_token` (~45줄)
- [ ] `anygarden/auth/dependencies.py`: `Identity`, `get_identity`, `require_room_*` (~40줄)
- [ ] `anygarden/db/models.py`: `AgentToken` 테이블 추가 (~15줄)
- [ ] `anygarden/cli.py`: `admin token create` 명령 (~20줄)
- [ ] 테스트:
  - [ ] JWT 발급/검증 정상 경로
  - [ ] 만료된 토큰 거부
  - [ ] Room 스코프 외 접근 거부
...
| **Agent API Token** | Agent 인스턴스 | Agent 프로세스 환경변수 `ANYGARDEN_TOKEN` | 특정 Agent의 Room 참여 | 발급 경로에 따라 두 가지 (아래 주석) | `/ws/rooms/{id}` |
| **Machine Token** | Machine Daemon | `~/.anygarden/machine.token` (chmod 600) | 자기 Machine 제어만 | 장명 (회전 권장) | `/ws/machines/{id}` |
...
- **Untrusted 호스트는 standalone 모드만**: Daemon을 띄우지 말고 사용자가 `uvx anygarden-agent` 직접 기동. Daemon 경로 자체가 없음

민감한 것은 **처음부터** 신뢰도 높은 소수의 Machine에만 두어야 한다. 침해가 시작된 뒤에는 이 결정을 되돌릴 수 없다.

#### (c) 탐지 — 침해 발생 인지 (침해 중)

- **`anygarden_auth_anomaly_total` 지표**: 병렬 접속 급증, 가장 접속 시도 감시. **공격자가 정상 agent를 흉내 내면 효과 제한적**
- **파일 무결성 모니터링** (AIDE, Tripwire): `~/.anygarden/machine.token`, 바이너리, systemd unit 해시 감시
...
- `anygarden/auth/machine_token.py` (신규, ~60줄): 발급/검증/회전
- `anygarden/auth/dependencies.py`에 `get_machine_identity` Dependency 추가 (~30줄)
- `anygarden/db/models.py`에 `MachineToken` 테이블 추가 (~20줄)
...
```python
# anygarden_machine/daemon.py — 올바른 예
async with websockets.connect(
    ws_url,
