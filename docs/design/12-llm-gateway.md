...
> doorae-server 가 LiteLLM Proxy 를 서브프로세스로 내장 관리하고, 모든 에이전트의 LLM 호출이 `/api/v1/llm/*` 역프록시 경로를 경유하게 한다. 이 계층은 §10 Machine 스케줄링 위에 **얹히는** 선택적 계층이며, Feature flag (`DOORAE_LLM_GATEWAY_ENABLED`) 로 on/off 된다.

배경과 대안 비교는 [ADR-004](../decisions/004-embedded-litellm-gateway.md), 상세 구현 계획은 [`.tmp/plan-197-embedded-litellm-gateway.md`](../../.tmp/plan-197-embedded-litellm-gateway.md), 이슈는 [#197](https://github.com/e7217/doorae/issues/197).

## 12.0 요약

**한 줄**: "doorae-server 가 LiteLLM 을 자식 프로세스로 돌리고, 에이전트의 LLM 호출은 전부 doorae 의 `/api/v1/llm/*` 를 거친다."
...
2. **사용량 추적이 비어 있다**. 호출이 doorae 를 거치지 않으므로 룸/에이전트/모델별 집계가 불가능.
3. **프로토콜이 엔진마다 다르다**. Claude Code 는 Anthropic `/v1/messages`, Codex 는 OpenAI `/v1/chat/completions`. 둘 다 커버하려면 단순 reverse proxy 로 부족.

### 무엇으로 푸는가

LiteLLM Proxy (OSS, 1.83+) 가 두 프로토콜을 같은 포트의 서로 다른 path 로 동시 서빙한다. 이를 doorae-server 의 **서브프로세스로** 묶어서, 설치·운영 아티팩트를 늘리지 않고 기능만 얻는다.
...
2. **stateless 운용**. LiteLLM 의 Postgres 의존 기능 (virtual key UI, persistent spend tracking) 은 포기. 사용량은 doorae SQLite 에 직접 기록.
3. **외부 노출 없음**. LiteLLM 은 `127.0.0.1:4001` 만 listen. 유일한 접근 경로는 doorae-server 의 역프록시.
...
```mermaid
sequenceDiagram
    participant Agent as doorae-agent<br/>(머신 B)
...
admin: POST /api/v1/llm-gateway/apply
  ...
  ├─ 2. atomic write to ~/.doorae/litellm.yaml
...
```
subprocess spawn 시 `env=` 로 `DOORAE_LITELLM_ANTHROPIC_API_KEY=<복호화된 값>` 주입. 파일엔 참조만, 프로세스 메모리에만 평문.

Fernet 키는 기존 `DOORAE_MCP_SECRETS_KEY` 재사용 — 운영자가 관리해야 할 KMS 키 1 개 유지.
...
- doorae-server 의 `subprocess.Popen(["litellm", ...])` 이 wrapper 없이 동작
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `DOORAE_LLM_GATEWAY_ENABLED` | `false` | 게이트웨이 기동 여부 |
| `DOORAE_LLM_GATEWAY_PORT` | `4001` | LiteLLM subprocess 의 listen 포트 |
| `DOORAE_LLM_GATEWAY_CONFIG_PATH` | `~/.doorae/litellm.yaml` | 렌더링된 config 위치 |
| `DOORAE_MCP_SECRETS_KEY` | (기존) | Fernet 키. 시크릿 암호화에 재사용 |
...
- LiteLLM subprocess 의 stdout/stderr 는 doorae-server 의 logger 에 relay (structlog 로 prefix `llm_gateway.stdout`)
...
현재 `packages/agent/doorae_agent/secrets.py:135-157` 에 `secrets_in_env` 가 정의·테스트만 되고 production 호출이 없다. 이 계층을 실제로 쓰는 첫 사용자가 게이트웨이다.
...
서버 측 매니페스트 생성에서 `DOORAE_LLM_GATEWAY_ENABLED=true` + "이 에이전트는 게이트웨이 경유" 설정 시:

```python
engine_secrets = {
    "ANTHROPIC_BASE_URL": f"{server_base_url}/api/v1/llm",
    "ANTHROPIC_AUTH_TOKEN": agent.doorae_token,
    "OPENAI_BASE_URL":    f"{server_base_url}/api/v1/llm/v1",
    "OPENAI_API_KEY":     agent.doorae_token,
}
```
...
- **토큰 도난**: 에이전트 토큰이 유출되면 해당 에이전트 명의로 LLM 호출 가능. doorae 의 기존 토큰 rotation 으로 대응 (게이트웨이 고유 이슈 아님).
- **시크릿 유출**: config.yaml 엔 평문 없음. env 는 서브프로세스 메모리만. `DOORAE_MCP_SECRETS_KEY` 유출 시엔 모든 암호문이 복호화되지만 이는 기존 MCP 시크릿과 동일 threat surface.
- **비용 폭탄**: 토큰 도난 시 admin 이 승인한 모델 한도 내에서 호출. 게이트웨이 단 rate limiting 은 추후 확장.

### 미처리 이슈 (명시)

- 에이전트 간 격리 — 같은 머신의 두 에이전트가 서로의 토큰을 훔칠 수 있는 경로는 여전히 존재 ([#184](https://github.com/e7217/doorae/issues/184) 에서 일부 완화, MCP subprocess 경로는 여전히 열려 있음). 게이트웨이가 이 문제를 직접 풀지는 않지만, 토큰 → master key 치환 덕분에 **실제 API 키** 는 새지 않는다. 사고 시 복구 비용이 줄어드는 간접 효과.
...
- 이슈: [#197](https://github.com/e7217/doorae/issues/197)
