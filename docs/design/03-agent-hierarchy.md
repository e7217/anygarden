...
## 3.2 SQLAlchemy 모델 (`anygarden/db/models.py` 발췌)
...
from anygarden.db.models import Base  # DeclarativeBase
...
## 3.3 서비스 레이어 (`anygarden/rooms/service.py` 발췌)
...
from anygarden.db.models import Room, Participant
from anygarden.rooms.errors import NotMember, RoomNotFound, SubRoomLoopError
...
sequenceDiagram
    ...
    participant SDK_PM as anygarden-sdk (PM)
    participant Server as anygarden-server
    participant DB as SQLite
    participant SDK_D as anygarden-sdk (Designer)
    ...
    Note over SDK_D: anygarden-sdk가 자동으로 새 Room 구독
...
- [ ] `anygarden/db/models.py`: `Room.parent_room_id`, `Room.is_dm` 추가 (~10줄)
- [ ] `anygarden/db/migrations/versions/xxxx_add_parent_room_id.py`: Alembic 마이그레이션 (~15줄)
- [ ] `anygarden/rooms/service.py`: `create_sub_room()` 함수 (~30줄)
- [ ] `anygarden/rooms/errors.py`: `NotMember`, `RoomNotFound`, `SubRoomLoopError` (~10줄)
- [ ] `anygarden/ws/handler.py`: `create_sub_room` 프레임 핸들러 (~15줄)
- [ ] `anygarden/ws/protocol.py`: `CreateSubRoomFrame` Pydantic 모델 (~10줄)
- [ ] `anygarden/rooms/router.py`: `GET /api/v1/rooms/{id}/children` 엔드포인트 (~15줄)

### SDK 측

- [ ] `anygarden_sdk/client.py`: `create_sub_room()` 메서드 (~20줄)
- [ ] `anygarden_sdk/tools.py`: `@expose_tool` 데코레이터로 LLM에 노출 (~30줄)
- [ ] `anygarden_sdk/client.py`: `on_participant_joined` 이벤트 → 자동 subscribe (~15줄)
...
2. **(§10)** PM 에이전트가 관리 API로 Designer 에이전트를 선언적 생성:
   ...
   ```
3. **(§10)** 서버 스케줄러가 Machine 선택 → 해당 Machine의 Daemon이 `uvx anygarden-agent` subprocess spawn → Agent가 서버에 독립 WS 연결
