...
# 채팅 서버 연결 (MCP와 독립)
client = ChatClient(url="wss://anygarden.example.com/ws", token="agt_...")
...
**관찰**:
- `mcp_servers=[...]` 는 Claude Code SDK의 설정이다. Anygarden 서버는 이 설정의 존재조차 모른다.
...
**모든 경우에 공통**: MCP 설정은 에이전트 엔진의 영역이며, Anygarden 서버는 이를 모른다. SDK도 MCP를 직접 다루지 않는다.
...
```python
# anygarden/messages/service.py (발췌)
...
**A**: 각 에이전트는 독립된 MCP 클라이언트를 가진다. 공유하려면 B의 프로필 파일(`~/.anygarden/agents/b.yaml`)에 같은 MCP 서버를 추가한다.

### Q5: Anygarden 서버가 MCP 서버 디스커버리를 돕나?

**A**: **아니다**. 디스커버리는 에이전트 프로필 파일(`~/.anygarden/agents/*.yaml`)에서 관리된다. 서버는 에이전트의 MCP 설정을 모르며, 알 필요도 없다.
...
- [ ] `anygarden/db/models.py`: `Message.extra_metadata` JSON 컬럼 존재 확인 (이미 포함)
- [ ] `anygarden/ws/protocol.py`: `SendMessageFrame.metadata` 필드 존재 확인 (이미 포함)
- [ ] `anygarden/messages/service.py`: `append_message()`에서 metadata를 검증 없이 저장 (이미 구현)
- [ ] `anygarden/ws/manager.py`: 브로드캐스트 시 metadata 포함 (이미 구현)
- [ ] **문서화**: README.md에 "서버는 MCP를 모른다. MCP는 각 에이전트 엔진이 자체 처리" 문장 명시

SDK 측 구현:

- [ ] `anygarden_sdk/integrations/*.py`: 엔진별 통합 시 MCP 설정은 **그대로 통과**시킨다. SDK는 MCP를 해석하지 않는다.
- [ ] `anygarden_sdk/examples/`: MCP를 사용하는 에이전트 프로필 예시 파일 2-3개 제공
...
**한 문장 정리**: Anygarden 서버는 채팅 프로토콜이고, MCP는 도구 프로토콜이다. 둘은 **목적이 다르며 경계를 넘지 않는다**. 이 경계를 지키는 것이 Plan A의 경량성을 유지하는 가장 중요한 실천이다.
