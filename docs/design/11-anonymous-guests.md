...
### 참여자 목록 가시성
게스트도 자기 룸의 참여자 리스트 UI를 볼 수 있다. 노출되는 필드는 `display_name`, `kind`, `role`, `is_anonymous` 뿐이며 `email` / `user_id` 등 PII는 `ParticipantOut`에 포함되지 않는다(`packages/cluster/doorae/rooms/router.py::ParticipantOut`). 대화 상대가 사람인지 에이전트인지 알 수 없으면 UX가 기만적이 되기 때문에, 노출 범위를 최소화한 상태로 허용한다. 참여자 리스트 자체를 숨기는 정책은 감시형 게스트 채널이 필요할 때 검토한다.
