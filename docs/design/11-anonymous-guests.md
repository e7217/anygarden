...
> Status: **Draft (RFC #22)**
> Owner: cluster team
> Tracking issue: https://github.com/e7217/anygarden/issues/22
...
```python
# packages/cluster/anygarden/db/models.py (변경)
class User:
    id: str
...
4. guest WS → wss://.../ws/rooms/{room_id}
              Sec-WebSocket-Protocol: anygarden.v1, bearer.<jwt>
```
...
### 참여자 목록 가시성
게스트도 자기 룸의 참여자 리스트 UI를 볼 수 있다. 노출되는 필드는 `display_name`, `kind`, `role`, `is_anonymous` 뿐이며 `email` / `user_id` 등 PII는 `ParticipantOut`에 포함되지 않는다(`packages/cluster/anygarden/rooms/router.py::ParticipantOut`). 대화 상대가 사람인지 에이전트인지 알 수 없으면 UX가 기만적이 되기 때문에, 노출 범위를 최소화한 상태로 허용한다. 참여자 리스트 자체를 숨기는 정책은 감시형 게스트 채널이 필요할 때 검토한다.
...
| 이름 | 정의 |
|---|---|
| `anygarden_guest_active` | 현재 WS 접속 중인 게스트 수 (ConnectionManager 기반, gauge) |
| `anygarden_guest_registered_total` | `is_anonymous=True, revoked=False` 게스트 수 (gauge, slow path) |
| `anygarden_guest_messages_total{room_id}` | 게스트가 보낸 메시지 총량 (counter) |
| `anygarden_invite_created_total{room_id}` | 발급된 invite 수 (counter) |
| `anygarden_invite_uses_total{invite_id}` | invite별 사용 횟수 (counter) |
| `anygarden_guest_rate_limited_total{scope}` | rate limit 차단 (scope=`cooldown`/`mention`/`room_aggregate`) |
...
| 관심사 | 파일 |
|---|---|
| 게스트 행 스키마 | `packages/cluster/anygarden/db/models.py` (`User.is_anonymous`, `User.display_name`, `RoomInviteLink`) |
| Invite API | `packages/cluster/anygarden/api/v1/invites.py` |
| 게스트 토큰 codec | `packages/cluster/anygarden/auth/jwt.py` (`GuestClaims`, `create_guest_token`, `verify_guest_token`, `decode_any_user_token`) |
| 게스트 수용 / 발급 | `packages/cluster/anygarden/auth/routes.py` (`POST /api/v1/auth/guest`) |
| 가드 | `packages/cluster/anygarden/dependencies.py` (`forbid_guest`), `anygarden/auth/dependencies.py` (`require_room_member` 게스트 분기) |
| WS 필터 / rate limit | `packages/cluster/anygarden/ws/handler.py`, `anygarden/orchestration/rules.py` (`GuestRoomAggregateLimiter`) |
| 정리 잡 | `packages/cluster/anygarden/guest/anonymize.py` (`python -m anygarden.guest.anonymize --db-url …`) |
| 메트릭 | `packages/cluster/anygarden/observability/metrics.py` (`guest_active`, `invites_created_total`, `invites_used_total`, `guest_rate_limited_total`) |
...
운영 체크리스트:
- `anygarden.guest.anonymize`를 일 1회 cron으로 예약 (기본 30일 grace, `--after-hours`로 조정 가능)
...
- **옵션 2**: C에 기본 *deny-all-guests* 플래그(`ANYGARDEN_ENABLE_GUESTS=false`)를 넣어 C 단독 머지 후 D/E 머지 완료 시점에 플래그를 켠다. 운영 쪽에서 선호.
