...
```python
# anygarden/messages/service.py (PG 분기)
...
async def room_ws(
    ...
    # Subprotocol 협상: 클라이언트가 보낸 "anygarden.v1, bearer.<token>" 중
    # "anygarden.v1"을 선택하여 반환한다.
...
```python
# anygarden_sdk/integrations/base.py
...
3. 에이전트가 재시작되면 `~/.anygarden/agents/{name}.yaml` 프로필 재로드 → 이전에 참여했던 Room에 재접속
...
이 수치는 Anygarden의 현실적 초기 운영 시나리오(소규모 팀 ~100명, 스프린트 단위 Room ~20개)에 기반한다.

### 7.4.2 서버 내부 SLO (Service Level Objectives)

| 지표 | p50 | p99 | 측정 방법 |
|---|---|---|---|
| 메시지 라우팅 지연 (WS 수신 → fanout 전송) | <5ms | **<50ms** | `anygarden_ws_fanout_duration_seconds` |
| DB write 지연 (Message INSERT) | <2ms | **<10ms** | `anygarden_db_write_duration_seconds` |
| 재연결 복구 지연 (since_seq 쿼리 + 전송) | <20ms | **<200ms** | `anygarden_reconnect_recovery_duration_seconds` |
| WebSocket 핸드셰이크 지연 | <50ms | **<500ms** | `anygarden_ws_handshake_duration_seconds` |
...
```python
# anygarden/observability/metrics.py
...
WS_FANOUT_DURATION = Histogram(
    "anygarden_ws_fanout_duration_seconds",
