...
> **한 줄 요약**: 단일 Python 프로세스 서버 + 임베디드 SQLite + WebSocket 단일 프로토콜 + 패키지 스크립트 CLI. `uvx anygarden-server` 한 줄로 기동.
...
1. **서버 호스트** — `anygarden-server` 프로세스. 채팅 메시지 라우팅 + 영속화 + 스케줄링.
2. **Machine (N대)** — 각각 별도 PC/VM/서버. `anygarden-machine` 데몬이 상주하며, 서버의 명령에 따라 에이전트 subprocess를 spawn/kill한다. Machine은 서버와 **물리적으로 별개의 호스트**이며, 네트워크(WebSocket)로만 연결된다.
...
graph TB
    subgraph Users["유저 호스트"]
        ...
        CLI_User["uvx anygarden-client<br/>(TUI / 텍스트 UI)"]
    end

    subgraph ServerHost["서버 호스트 (별도 머신)"]
        direction TB
        APP["FastAPI app<br/>(anygarden.app)"]
...
- **`anygarden-machine` daemon이 Machine의 대표자**다. 서버에 자기 capability(어떤 엔진이 설치되어 있는지)를 보고하고, 서버의 `spawn_agent` 명령을 받아 로컬에서 `uvx anygarden-agent` subprocess를 실행한다.
...
```
anygarden-server/                    # 서버 레포지토리
├── pyproject.toml                # hatchling + [project.scripts] 엔트리포인트
├── README.md
├── LICENSE
├── anygarden/                       # Python 패키지 루트 (import anygarden)
│   ├── __init__.py               # __version__
│   ├── __main__.py               # python -m anygarden 지원
│   ├── cli.py                    # anygarden-server / anygarden-client CLI 엔트리
│   ├── config.py                 # Pydantic Settings (~/.anygarden/config.toml)
...
```
anygarden-sdk/                       # 별도 PyPI 패키지
├── pyproject.toml
├── anygarden_sdk/
│   ├── __init__.py
│   ├── client.py                 # ChatClient (WebSocket 래퍼 + 재연결)
│   ├── cli.py                    # anygarden-agent CLI
│   ├── protocol/
│   │   ├── frames.py             # 서버 anygarden/ws/protocol.py 와 동일한 Pydantic 모델
│   │   └── versioning.py
│   └── integrations/
│       ├── __init__.py
│       ├── base.py               # 공통 훅 인터페이스
...
- `anygarden-server`는 `anygarden-sdk`를 의존하지 않는다 (역방향 의존 없음).
- `anygarden-sdk`도 `anygarden-server`를 의존하지 않는다.
...
| 모듈 | 기대 LOC | 계층 | 역할 |
|---|---:|---|---|
| `anygarden/app.py` | 50 | 필수 | FastAPI 앱 팩토리, lifespan(startup/shutdown), CORS, 미들웨어 등록 |
| `anygarden/config.py` | 30 | 필수 | Pydantic Settings (DB_URL, JWT_SECRET, CORS, LOG_LEVEL, DATA_DIR) |
| `anygarden/db/engine.py` | 25 | 필수 | async engine + async session maker |
| `anygarden/db/models.py` | 130 | 필수 | 7개 엔티티 + 제약조건 + 인덱스 |
| `anygarden/db/repository.py` | 60 | 필수 | `append_message`, `replay_since_seq`, `list_rooms` |
| `anygarden/auth/jwt.py` | 40 | 필수 | JWT 인코딩/디코딩 |
| `anygarden/auth/token.py` | 45 | 필수 | API Token 발급·검증·argon2 해시 |
| `anygarden/auth/dependencies.py` | 40 | 필수 | `require_user`, `require_room_member`, `require_admin` |
| `anygarden/ws/manager.py` | 80 | 필수 | ConnectionManager (dict[room_id, set[WebSocket]]) |
| `anygarden/ws/handler.py` | 90 | 필수 | /ws/rooms/{id} 핸들러, 프레임 파싱, heartbeat |
| `anygarden/ws/protocol.py` | 50 | 필수 | Pydantic 프레임 (SendMessage, Message, JoinRoom, ...) |
| `anygarden/rooms/router.py` | 70 | 필수 | REST: rooms CRUD + sub-room 생성 |
| `anygarden/rooms/service.py` | 60 | 필수 | 권한 상속, parent_room 검증 |
| `anygarden/messages/router.py` | 40 | 필수 | REST: 히스토리 페이지네이션 |
| `anygarden/messages/service.py` | 50 | 필수 | `append_message(room, content)` + seq 발급 |
| `anygarden/orchestration/rules.py` | 50 | 필수 | 쿨다운 토큰 버킷, 멘션 파싱 |
| `anygarden/observability/metrics.py` | 30 | 필수 | 5개 Prometheus 지표 선언 |
| `anygarden/observability/logging.py` | 20 | 필수 | structlog JSON 설정 |
| `anygarden/cli.py` | 80 | 필수 | Typer/click CLI: server/client/admin |
| `anygarden/db/migrations/` | 80 | Tier 1 부속 | Alembic env.py + 초기 마이그레이션 |
| **필수 합계** | **~960** | | |
...
**SQLAlchemy 2.0 모델 예시** (`anygarden/db/models.py` 발췌):
...
나머지 `User`, `Agent`, `Machine`, `Participant` 모델도 동일한 스타일로 `anygarden/db/models.py`에 구현한다.
...
`anygarden/ws/protocol.py`의 Pydantic v2 모델로 정의한다. JSON 텍스트 프레임만 사용 (바이너리 없음).

> 이 표는 `packages/cluster/anygarden/ws/protocol.py`의 실제 모델과 1:1 매핑된다. 추가/변경 시 양쪽을 함께 갱신할 것. 룸 멤버 인증/구독은 별도 REST가 아니라 WS 연결 시점의 `Sec-WebSocket-Protocol: anygarden.v1, bearer.<token>` 서브프로토콜과 `?since_seq=...` 쿼리로 처리된다 (`anygarden/ws/handler.py`).
...
```python
# packages/cluster/anygarden/ws/protocol.py
...
│  단일 머신 (개발자 랩탑)                │
│                                         │
│  Terminal 1: uvx anygarden-server          │
│  Terminal 2: uvx anygarden-agent (PM)      │
│  Terminal 3: uvx anygarden-agent (Coder)   │
│  Terminal 4: uvx anygarden-client (유저)   │
│                                         │
│  파일: ~/.anygarden/anygarden.db (SQLite)     │
│       ~/.anygarden/config.toml             │
│       ~/.anygarden/agents/*.yaml           │
└─────────────────────────────────────────┘
```
...
│                                          │
│  systemd --user                          │
│   └─ anygarden-server.service               │
│       └─ uvx anygarden-server (WAL SQLite)  │
│                                          │
│  nginx (또는 Caddy)                      │
...
│                                          │
│  /usr/local/bin/anygarden-server            │
...
tui = ["textual>=0.50"]  # anygarden-client TUI
...
## 1.8 설정 파일 (`~/.anygarden/config.toml`)

`anygarden/config.py`가 읽는 기본 경로는 `~/.anygarden/config.toml`이다. 없으면 기본값으로 동작한다.

```toml
# ~/.anygarden/config.toml (예시)
[server]
host = "127.0.0.1"
port = 8000
log_level = "INFO"
...
[db]
# SQLite 기본. PG로 승격 시: "postgresql+asyncpg://user:pass@host/anygarden"
url = "sqlite+aiosqlite:///~/.anygarden/anygarden.db"
pool_size = 5

[auth]
jwt_secret_file = "~/.anygarden/jwt.secret"   # 파일이 없으면 첫 실행 시 자동 생성
jwt_expire_hours = 24
...
환경 변수로도 오버라이드 가능하다 (`ANYGARDEN_SERVER_PORT=9000` 등, pydantic-settings 자동 매핑).
...
```
브라우저 WS send
  → anygarden/ws/handler.py: 프레임 파싱 (Pydantic)
  → anygarden/auth/dependencies.py: Room 멤버 검증 (캐시)
  → anygarden/orchestration/rules.py: 쿨다운 체크
  → anygarden/messages/service.py: append_message + seq 발급 (트랜잭션)
  → anygarden/ws/manager.py: Room 구독자 전원에 broadcast
...
| 계층 | 담당 파일 | 역할 |
|---|---|---|
| 채팅 레이어 (이 문서) | `anygarden/ws/handler.py` 등 | Room, Message, Participant, Subagent channel |
| **스케줄링 레이어 (§10)** | `anygarden/scheduler/`, `anygarden/ws/machine_handler.py` | Machine 등록, Daemon ↔ 서버 WS, Agent subprocess 생성/종료, bin-pack placement |
...
| 저장소 | import | 설명 |
|---|---|---|
| `anygarden-server/` | `anygarden` | 이 문서의 메인 서버 (변경 없음, `anygarden/scheduler/`와 `anygarden/ws/machine_handler.py` 추가) |
| `anygarden-sdk/` | `anygarden_sdk` | 에이전트 프로세스가 사용하는 클라이언트 SDK (변경 없음) |
| **`anygarden-machine/`** (신규) | `anygarden_machine` | **머신 호스트에 상주하는 Daemon. 엔진 감지, subprocess 관리** |
...
§1.3의 LOC 표에 Machine 스케줄링 계층을 추가한다. §1-§9 본체의 **확정 기준선은 ~960**이고 (§1.3), 여기에 §10의 Machine 스케줄링 구현 범위(~370-790)를 더해 `anygarden-server` 본체 전체 합계는 **~1,330-1,750** 범위에 들어온다. 범위의 폭은 "최소 필수 구현(§10.7 기준)과 여유 포함 상한(§10.13 기준)"의 차이다.

| 카테고리 | §1-§9 | §10 추가 | 합계 |
|---:|---:|---:|---:|
| `anygarden-server` 본체 | ~960 | ~370-790 | ~1,330-1,750 |
| `anygarden-sdk` | ~400 | — | ~400 |
| `anygarden-machine` (신규 패키지) | — | ~410 | ~410 |
