...
| 후보 | 장점 | 단점 |
|---|---|---|
| **Python** | FastAPI 성숙도, 에이전트 SDK 생태계 Python 편향, 기존 Doorae 코드 재사용, uvx 네이티브 | 단일 바이너리 크기 20-50MB, 시작 100-500ms, 메모리 50-100MB |
| **Go** | 단일 바이너리 10-20MB, 시작 <10ms, 메모리 5-15MB, 동시성 우수 | 기존 Doorae Python 코드 폐기, 에이전트 SDK는 대부분 Python, 개발 속도 2-3배 느림 |
...
3. **개발 속도**
   ...
   - Doorae는 MVP 속도가 **사용자가 가장 요구하는 것**이다. Plan A의 정체성이 "1-2주 MVP"인데 여기서 언어 교체는 모순이다.

4. **uvx 네이티브**
   - `uvx`는 PyPI 패키지를 일회성 실행하는 유일한 표준 툴이다.
   - Go는 `go install`이 있지만 이 경우 사용자에게 Go 툴체인을 요구한다.
   - Python + `uv`는 "사용자가 `uvx doorae-server`만 알면 된다" 수준의 UX를 제공한다.
...
1. **"Docker 없음" 요구와의 정합성**
   ...
   - SQLite는 `uvx doorae-server` 한 줄로 완결된다. DB 프로세스가 별도로 존재하지 않는다.
...
3. **관측·백업 단순성**
   - `cp ~/.doorae/doorae.db backup-20260406.db` — 백업 끝.
   - `sqlite3 doorae.db ".dump" > backup.sql` — 평문 덤프.
...
| URL | `sqlite+aiosqlite:///~/.doorae/doorae.db` | `postgresql+asyncpg://...` | `.toml` |
...
3. **관측성**
   ...
   - `websocat ws://localhost:8000/ws/rooms/xxx --protocol 'doorae.v1,bearer.<token>'` 한 줄로 수동 테스트 가능. 토큰은 `Sec-WebSocket-Protocol` 헤더로 전달하며 URL 쿼리에는 넣지 않는다(05-security.md §5.7.1).
...
- **Phase 1 (MVP)**: Python SDK (`doorae-sdk` on PyPI) **단독**
- **Phase 2**: TypeScript SDK (`@doorae/sdk` on npm) 추가
...
- **1차 배포 수단**: `uvx doorae-server` (PyPI 패키지)
...
|---|---|---|---|---|
| `uvx` | `uv` 설치 (`curl -Ls https://astral.sh/uv/install.sh | sh`) | `uvx doorae-server` | 패키지 다운로드 ~10MB | ~10초 (첫 실행) / <1초 (캐시) |
| `pip install + script` | Python + pip | `pip install doorae-server && doorae-server` | ~10MB | ~30초 |
| `pipx` | pipx 설치 | `pipx install doorae-server` | ~10MB | ~30초 |
| PyInstaller 바이너리 | 없음 | `./doorae-server` | 20-50MB | <1초 |
| Docker | Docker | `docker run doorae/server` | ~100MB 이미지 | ~5초 |
...
- **컨텍스트**: Doorae 멀티 에이전트 채팅 서버 구현 언어 결정
...
- **컨텍스트**: 초안 impl/에서 Machine을 수동 메타데이터로만 취급하여, 사용자가 매번 `uvx doorae-agent`를 수동 실행하는 전제였음. 그러나 원래 요구사항("프로젝트·머신·에이전트·유저" 4대 엔티티 + "호스트당 복수 에이전트")은 **Machine을 활성 참여자**로 다룰 것을 암시함. 수동 운영은 1-3 머신 로컬 테스트에는 맞지만, "알리스는 노트북에 2개 + 밥은 GPU 머신에 3개" 같은 분산 배치가 불가능.
- **결정**: Machine은 `doorae-machine` Daemon을 통해 서버에 등록되는 **1급 스케줄링 리소스**. 서버는 Machine을 대상으로 "이 엔진으로 에이전트 띄워라"를 선언적으로 명령하고, Daemon이 로컬 subprocess를 관리한다. Daemon 없이 수동 실행하는 **standalone 모드도 폴백으로 유지**한다.
- **결과**:
  ...
  - 3번째 패키지 `doorae-machine` 추가 (~410 LOC)
  - MVP 기간 2-3주 → 4-5주
- **대안 기각**:
...
- **완화책**:
  - Standalone 모드(수동 `uvx doorae-agent`)를 그대로 유지하여 소규모/로컬 테스트 경로 보존
  - 서버 본체 복잡도 증가는 명확한 모듈 분리 (`doorae/scheduler/`)로 국지화
